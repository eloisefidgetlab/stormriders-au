...
(function() {
		...
		if (images.length > 0) {
			...
			if(tags.length){
				for (var k = 0; k < tags.length; k++) {
					var tag = tags[k];
					if (tag.indexOf('flag-bundle-deals:') != -1) {
						htmlImg += ' <div class="product-tag"><span class="product-tag--bundle-deals">' + tag.split('flag-bundle-deals:')[1] + '</span></div>';
						break;
					}else if (tag.indexOf('flag-instore') != -1) {
						htmlImg += ' <div class="product-tag"><span class="product-tag--instore-only">INSTORE ONLY</span></div>';
						break;
					}else if (tag.indexOf('flag-default:') != -1) {
						htmlImg += ' <div class="product-tag"><span class="product-tag--default">' + tag.split('flag-default:')[1] + '</span></div>';
						break;
					}
				}
			}
	...
	//'More Colours' link when a product has multiple colour variants
	function buildMoreColoursLink(data){
		var html = '';
		data.options.forEach(function (option, index) {
			...
			data.variants.forEach(function (variant, index) {
				var current_option = variant.options[option_index];
				if(!(current_options.includes(current_option))){
					if(how_many_options > 0){
						current_options = current_options.join('|').concat('|');
					}

					current_options = current_options.concat(current_option);
					current_options = current_options.split('|').sort();
					how_many_options = current_options.length;
				}

			});

            if(current_options.length > 1 && option.includes("colo")){
				html += '<a href="{{itemUrl}}">' + '+ More Colours' + '</a>';
			}
		});
		return html;
	}
	...
	function buildInfo(data, indx) {
		...
		if (boostPFSThemeConfig.custom.show_product_info) {
			...
			html += buildMoreColoursLink(data); //'More Colours' link when a product has multiple colour variants
